...
public interface FillBlankQuestionRepository extends JpaRepository<FillBlankQuestion, String> {
    ...
    List<FillBlankQuestion> findByChannelAndLangAndIsActiveTrueOrderByDisplayOrderAsc(String channel, FillBlankQuestion.Language lang);

    boolean existsByQuestionAndChannelAndLang(String question, String channel, FillBlankQuestion.Language lang);

    @Query("SELECT COUNT(q) > 0 FROM FillBlankQuestion q WHERE q.question = :question AND q.channel = :channel AND q.lang = :lang AND q.id != :excludeId")
    boolean existsByQuestionAndChannelAndLangExcludingId(
            @Param("question") String question,
            @Param("channel") String channel,
            @Param("lang") FillBlankQuestion.Language lang,
            @Param("excludeId") String excludeId
    );

    @Query("SELECT q FROM FillBlankQuestion q WHERE " +
